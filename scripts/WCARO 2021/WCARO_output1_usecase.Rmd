...
Access to an improved water source is the first step towards achieving safe water coverage for a population. When different methodologies are used to report coverage, wide discrepancies appear in the reported figures for people with access to safe water and those left unserved. However, planners and policy makers require correct and updated information to estimate the real need and to assess the scale of vulnerabilities across unserved / underserved populations, as well as make projections for future allocations. In this overview, we demonstrate how such differences appear by using three different ways of estimating water access in the region of Kambia.  

# Methodology
Many  countries continue to face challenges in recording and updating their water coverage data, either due to lack of resources or the capacity to collect information about SDG parameters. Generally, during reporting, measurements of water coverage seldom take population density into account. However, using our example of data from Kambia we can highlight the extent of differences that appear in coverage figures when the population density is also taken into account.

## Measuring coverage based on *sphere guidelines*^[https://spherestandards.org/wp-content/uploads/Sphere-Handbook-2018-EN.pdf]

The following table shows the water points in the district Kambia at time of the 2016 water point mapping. The three parameters used to describe the different categories of water points include:
...
- Total water points (totalImproved and unimproved)
- Functional / not functional water points

The coverage has been determined using the Sphere guidelines, which are guidelines to determine the number of people a specific type of water point can be expected to serve, if it functions well. 
...
pie_data_1 <- data.frame(methodology = c("Ministry data improved water points",
...
pie_data_1 <- pie_data_1  %>%
  ...
  mutate(methodology = factor(methodology,
                              levels = c("Ministry data improved\nwater points",
                                         "Ministry data improved\nand functional water\npoints")))
...
As you can see from table 1 Kambia reported  750 water points that have the potential  to cover about 0.29 million people. Of these water points 686 have been categorised as “improved”, and can be able to serve approximately 0.27 million people, see table 2.

<br> `r table2` <br> 

Table 3 shows that of the 750 water points, 433 reported to be functional at time of the data collection, and table 4 shows that out of these 433 functional points, 399 were reported in the  “improved” category. 

<br> `r table3` <br> 

<br> `r table4` <br>

Based on a national inventory in 2016^[https://washdata-sl.org/water-point-data/], there were approximately`r pop_projections[pop_projections$ADM2 == "Kambia","2016"]` people living in Kambia. Going by the projections,  population figures of Kambia were expected to grow up to `r pop_projections[pop_projections$ADM2 == "Kambia","2018"]` by 2018.  If we were to measure coverage by simply looking at the reported population in Kambia, `r round(291950/pop_projections[pop_projections$ADM2 == "Kambia","2016"],2)*100`% of the population would be shown as ‘served’ by the 750 (total) water points. When the same data is  distinguished between improved and unimproved categories, only 77% of the population in Kambia was found to be served by improved water points, see table 5. When the data is further disaggregated by functionality, i.e. whether the water points are functional or not, the coverage proportions are further reduced to 43%. This analysis helps us to understand the ‘real coverage’ and asserts the need for disaggregating data by type of source and functionality to decide the ‘real need’ for water supply across populations.
...
```{r pie1, fig.align="center", fig.width=6, fig.height=6, fig.cap="Figure 1: Estimated basic water coverage  ", echo=FALSE, message=FALSE, warning=FALSE}
pie_data_1 <- pie_data_1 %>%
  mutate(variable = recode(
    variable, 
    percentage_served = "Projected\npercentage served", 
    percentage_not_served = "Projected\npercentage not served"))

ggplot(data = pie_data_1, aes(x = "", y = value, fill=variable )) + 
  geom_bar(stat = "identity", position = position_fill()) +
  scale_fill_manual(values = c( "#7868E6", "#B8B5FF")) +
  geom_text(aes(label = paste0(value, "%")), position = position_fill(vjust = 0.5), size=6) +
  coord_polar(theta = "y") +
...
```{r example_2, echo=FALSE, message=FALSE, warning=FALSE}
# Shape file data
...
Kambia is about 3000 square kilometers and is estimated to have about 800 villages. When you take into account the requirement of the JMP that water should be collectible within a 30 minute round trip, it is important to know if the water points are dispersed in a way that fits the population density.
...
```{r pop_density_and_wp, fig.show="hold", out.width="50%"}
...
ggplot() +
  ...
  labs(title="Population density in Kambia", color="")
...
ggplot() +
  ...
  labs(title="Water point improved status", x="", y="") +
...
ggplot() +
  ...
  labs(title="Water point functionality status", x="", y="") +
  theme(legend.position = "right",
        legend.title = element_blank(),
        legend.text = element_text(size=8),
        plot.title = element_text(size=14),
...
```{r graphs_combined,  echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot() +
  ...
  geom_point(data=wp_2016_sub, 
             aes(x=Longitude, y=Latitude, fill=sdg_improved_source), shape=25,  stroke = 0.2, alpha=0.8)  +
  scale_fill_manual(values=c("#1EAFED", "#FFCD60")) +
  labs(title="Kambia: population density and water points", x="", y="") +
  theme(legend.position = "right",
        legend.title = element_blank(),
        legend.text = element_text(size=8),
        plot.title = element_text(size=14),
...
ggplot() +
  ...
  labs(title="Kambia: population density water points improved and functional", x="", y="") +
  theme(legend.position = "right",
        legend.title = element_blank(),
        legend.text = element_text(size=8),
        plot.title = element_text(size=14),
...
The map below shows the location of the different water points in Kambia with the functionality status. The highest concentration of water points seems to be in the middle, and more to the south, and the lowest concentration is in the outer points of the district. Along the eastern border there seem to be quite some non functional water points.
...
ggplot() +
  ...
  labs(title="Projected percentage of the population served", color="")
...
pie_data <- improved_wp_coverage_fb_density %>% 
  ...
  mutate(percentage_not_served = 100 - percentage_served) %>% melt() %>%
  mutate(methodology = "Population density data, improved sources") %>% 
  full_join(
    functional_wp_coverage_fb_density %>% select(ADM2, "Actual percentage of the population served") %>%
      rename(percentage_served = `Actual percentage of the population served`) %>% 
      mutate(percentage_not_served = 100 - percentage_served) %>% melt() %>%
      mutate(methodology = "Population density data, improved and functional sources"))
