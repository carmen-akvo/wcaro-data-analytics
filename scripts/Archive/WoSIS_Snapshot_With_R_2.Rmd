...
This document shows how to access WoSIS "Snapshot" data from  R. For access to WoSIS "Latest" data from R, see https://www.isric.org/accessing-wosis-latest-r
...
The Procedures Manual https://www.isric.org/sites/default/files/isric_report_2018_01.pdf describes how the database was built.
...
library(dplyr)          # another way to handle tabular data
...
Unpack the file; this will take some time.

```{r}
system.time(
  unzip(target.zip, exdir=wosis.dir.name, junkpaths=TRUE)
)
...
dim(profiles.hi <- profiles %>% 
      filter(country_id=="NL") %>%
      filter(geom_accuracy < 1/3600))
...
* `date` -- one or more values, in the format {1:yyyy-mm-dd; 2:yyyy-mm-dd...}, which are the dates each of th eduplicate measurements was added to the database (not the original measurement date, nor the field sampling date)
...
Read in the physical attributes:

We use the `readr` function `read_tsv`, i.e. "read tab-delimited text file".
...
Read in the chemical attributes. Again we need to supply the optional `col_types` argument to specify the data type of each field.
...
The primary key in the profiles table is `profile_id`; the `profile_id` field in the two attribute tables  is the foreign key; this along with the `profile_layer_id` is the full key to the attribute tables.

It's simplest to select the profiles in the profile table, and use these profile IDs to select out of the attribute table. For example, to analyze the particle-size distribution of Indian soil profiles we first find the profiles from India:

```{r india.profiles}
(profiles.india <- filter(profiles, country_name=="India") %>%
    select(profile_id, longitude, latitude))
```

Then use the `left_join` function to add the layers to each profile. This will repeat the primary key `profile_id` for each value of the secondary key `profile_layer_id` (i.e., the same table structure as the table being joined), with any fields selected from the main (profile) table repeated.
...
For an introduction to Geopackage data structures, see "Getting Started With GeoPackage" ^[Getting Started With GeoPackage].

The geopackage stores data in SQL tables. To list these, use the `dplyr::src_sqlite` function:

```{r}
source <- paste0(wosis.dir.name, "/", "wosis_201909.gpkg")
(gpkg <- dplyr::src_sqlite(source))
```
...
```{r}
dplyr::tbl(src_sqlite(source), "gpkg_geometry_columns")
```
...
```{r}
dplyr::tbl(src_sqlite(source), "wosis_201909_profiles")
```
...
Read into an R spatial object as Simple Features [@Pebesma_2018] ^[https://en.wikipedia.org/wiki/Simple_Features].
Note that we must specify the optional `fid_column_name` argument to include the profile ID (primary key) as a column in the attribute table.
...
The profile-level attributes can also be plotted as maps, for example Soil Taxonomy Order in a tile:

```{r plot-st,fig.width=10, fig.height=6}
...
```{r}
(wosis.sf.india <- wosis.sf %>% filter(country_name=="India"))
...
Read the Geopackage into an R `sp` object [@Pebesma_Bivand_2005]. Note that by default `readOGR` reads the first layer from a Geopackage; here that is the profiles (the only layer with geometry).
...
```{r}
class(dplyr::tbl(src_sqlite(source), "wosis_201909_layers_chemical"))
class(dplyr::tbl(src_sqlite(source), "wosis_201909_layers_physical"))
```

Read these into R and display the variable names:

```{r}
wosis.chemical <- dplyr::tbl(src_sqlite(source), "wosis_201909_layers_chemical")
...
Here we convert the small dataset for India to a `SoilProfileCollection`.
...
plotSPC(ds.aqp[1:24,], name="layer_name", color='clay_value_avg')
```
