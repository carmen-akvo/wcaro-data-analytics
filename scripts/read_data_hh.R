...
## Read data
household <- read_excel(here("SL/Data", "SL_household.xlsx")) 
hh_monitoring <- read_excel(here("SL/Data", "SL_household_monitoring.xlsx")) 
...
SL_household <- SL_household %>% 
  mutate( location_Region = recode(
    location_Region,
    "5" = "North West"
  )) %>%
  mutate( location_District = recode(
    location_District,
    "1" = "Kailahun",
    "2" = "Kenema",
    "3" = "Kono",
    "6" = "Bonthe")) %>%
  mutate( location_Chiefdom = recode(
    location_Chiefdom,
    "1" = "Malema",
    "2" = "Mandu",
    "3" = "Njaluahun",
